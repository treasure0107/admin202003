    ...
    <div class="content">
        <div class="settled-in-search">
            <el-form :inline="true" :model="form">
                ...
                <el-form-item label="审核状态" prop="status" label-width="80px" >
                    <el-select
        ...
        <div class="table-content">
            <el-table
                ...
                <el-table-column prop="url" label="认证资料" width="220px">
                    <template slot-scope="scope">
                        <el-row v-if="scope.row.pic_url">
                            ...
                            <el-col :span="4"><div class="grid-content bg-purple-dark">
                                <preview-image v-if="scope.row.pic_url[3]" :image="scope.row.pic_url[3]" :height="'30px'" ></preview-image>
                            </div></el-col>
                            <el-col :span="4"><div class="grid-content bg-purple-dark">
                                <preview-image v-if="scope.row.pic_url[4]" :image="scope.row.pic_url[4]" :height="'30px'" ></preview-image>
                            </div></el-col>
                        </el-row>
                    </template>
                </el-table-column>
                ...
                <el-table-column prop="reviewer" label="审核人" show-overflow-tooltip></el-table-column>
            ...
            async checkStore(data){
                this.$confirm('是否设置为审核通过', '提示', {
                    ...
                    let result = await this.$request.request(this.$API.common_api,{
                        ...
                        request_host:'SHOP_CERTIFICATION_HOST',
                        handle_desc:"实地认证审核通过;"+data.shop_id,
                    });
            ...
            async checkCentent(data){
                this.$prompt('请输入审核原因', '提示', {
                    ...
                    let result = await this.$request.request(this.$API.common_api,{
                        ...
                        router_path: 'opp/train_info',
                        request_host:'SHOP_CERTIFICATION_HOST',
                        handle_desc:"实地认证审核不通过;"+data.shop_id,
                    });
                    SendMessage({message: '审核成功', type: 'success'});
                    this.dataList();
                }).catch(() => {
