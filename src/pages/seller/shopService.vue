    ...
    <div class="content">
        ...
        <div class="table-content">
            <el-table
                ...
                <el-table-column prop="shop_name" label="店铺名称" show-overflow-tooltip></el-table-column>
                <el-table-column prop="requirements" label="满足条件" width="220px" show-overflow-tooltip></el-table-column>
            ...
            async checkStore(data){
                this.$confirm('是否设置为审核通过', '提示', {
                    ...
                    let result = await this.$request.request(this.$API.common_api,{
                        ...
                        request_host:'SHOP_CERTIFICATION_HOST',
                        handle_desc:"金牌服务信息审核通过;"+data.shop_id
                    });
            ...
            async checkCentent(data){
                this.$prompt('请输入审核原因', '提示', {
                    ...
                    let result = await this.$request.request(this.$API.common_api,{
                        ...
                        router_path: 'opp/train_info',
                        request_host:'SHOP_CERTIFICATION_HOST',
                        handle_desc:"金牌服务信息审核不通过;"+data.shop_id
                    });
                    SendMessage({message: '审核成功', type: 'success'});
                    this.dataList();
                }).catch(() => {
