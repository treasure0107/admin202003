<template>
  <div class="content">
    <div class="pages-header">
      <div>秒杀管理</div>
      <el-button type="primary" round icon="el-icon-plus" size="medium" @click="addSeckill">创建秒杀</el-button>
    </div>
        ...
        <el-table  ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%"  @selection-change="handleSelectionChange">>
          ...
          <el-table-column fixed="right" label="操作" >
            <template slot-scope="scope">
              ...
              <el-button size="mini"  type="danger" v-if="scope.row.start_time <= now_time && scope.row.end_time >=now_time && scope.row.status==1" @click="end_activity(scope.row.aid)">结束秒杀</el-button>
              <el-button size="mini" @click="show_seckill_log(scope.row.aid)">操作日志</el-button>
            </template>
          </el-table-column>
        </el-table>
  ...
  export default {
    ...
    methods: {
      ...
      async get_seckill_list(){
        ...
        let {data} = await this.$request.post(this.$API.activity_seckill_list,
          param
          ,'main',
          {
            handle_desc:'查看秒杀列表;'
      ...
      show_goods_info(goods_id,shop_id,aid){
        window.open(this.ppy_host+'/customer/shop/'+shop_id+'/goods/2/'+aid+'/'+goods_id,'_blank');
      },

      show_seckill_log(aid){
        this.$router.push({
      ...
      async end_activity(aid){
        await this.$confirm('你确定要结束该秒杀活动吗，请谨慎操作', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(async () => {
          let res = await this.$request.post(this.$API.end_seckill,
            {aid:aid},
            'main',
            {
              handle_desc: '结束秒杀;'+aid
            });
          if(res.code==200) {
            SendMessage({
              message: '操作成功',
