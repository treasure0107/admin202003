<template>
  <div class="content">
    <div class="pages-header">
      <div><router-link to="/activity/seckill" style="text-decoration:none;color:#000000;">秒杀管理</router-link>/编辑秒杀</div>
    </div>
      ...
      <el-form  :model="addForm" ref="addForm"  class="demo-form-inline">
        ...
        <el-form-item>
          <el-table :data="skuArrList" stripe height="280" border style="width: 100%" @selection-change="handleSelectionChange">
            ...
            <el-table-column prop="activity_price" label="秒杀价格" width="220px">
              <template slot-scope="scope">
                <el-form-item  >
                  <el-col :span="14">
                    <el-input v-model="addForm.activity_price[scope.$index]" value="this.test2" placeholder="请输入秒杀价格"></el-input>
                  </el-col>
      ...
      submit_form_add(ruleForm){
        this.$refs[ruleForm].validate(async (valid) => {
          if (valid) {
            ...
            let res = await this.$request.post(this.$API.edit_seckill,
              this.form,
              '',
              {
                handle_desc:'编辑秒杀;'+this.form.aid
              });
            if(res.code==200){
              SendMessage({
                message: '操作成功',
