<template>
  <div class="content">
    <div class="pages-header">
      <div><router-link to="/activity/tg" style="text-decoration:none;color:#000000;">团购管理</router-link>/创建团购</div>
    </div>
      ...
      <el-form  :model="addForm" ref="addForm" :rules="rules" class="demo-form-inline">
        ...
        <el-form-item>
          <el-table :data="skuArrList" stripe height="280" border style="width: 100%" @selection-change="handleSelectionChange">
            ...
            <el-table-column label="团购价格" width="220px">
                <template slot-scope="scope">
                  <el-form-item  prop="activity_price" >
                    <el-col :span="14">
                     <el-input v-model="addForm.activity_price[scope.$index]" placeholder="请输入团购价格"></el-input>
                    </el-col>
  ...
  export default {
    data() {
      return {
        ...
        rules:{
          activity_price: [
            {required: true, message: '请填写团购价格', trigger: 'blur'},
          ],
          subsidy_precent: [
            {required: true, message: '请填写平台补贴比例'},
          ],
      ...
      submit_form_add(ruleForm,status){
        this.$refs[ruleForm].validate(async (valid) => {
          if (valid) {
            ...
            let res = await this.$request.post(this.$API.add_tg, this.form,'',{handle_desc:'新增团购;'});
            if(res.code==200){
              SendMessage({
                message: '操作成功',
                type:'success'
